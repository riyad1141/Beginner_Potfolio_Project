...
<script>
    ...
    contactForm.addEventListener('submit', async function (event) {
        ...
        if (name.length===0){
            ...
            alert('Please sir enter your email');
        }else if (phone.length===0){
            alert('Please sir enter your phone number');
        }else{
            let all_data = {
                fullName:name,
                email:email,
            ...
            let URL = "/contractRequest";

          //  showing loader and remove content

            document.getElementById('loading-div').classList.remove('d-none');
            document.getElementById('content-div').classList.add('d-none');

            let result = await axios.post(URL,all_data);
            ...
            if (result.status===200 && result.data===1){
                alert('Dear sir your form has been successfully submitted');
                contactForm.reset();
            }else{
